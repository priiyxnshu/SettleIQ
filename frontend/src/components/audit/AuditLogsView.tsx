...
export const AuditLogsView: React.FC = () => {
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <div className="bg-white px-3 py-1.5 rounded-xl border border-slate-200 shadow-2xs">
              ...
              <span className="font-semibold text-slate-900">{d.decided_by || 'Unknown Reviewer'}</span>
            </div>
            ...
            <div className="bg-white px-3 py-1.5 rounded-xl border border-slate-200 shadow-2xs">
              ...
              <span
                className={`font-semibold ${
                  isApproved ? 'text-emerald-700' : isRejected ? 'text-rose-700' : 'text-purple-700'
                }`}
              >
            ...
            {d.new_status && (
              <div className="bg-white px-3 py-1.5 rounded-xl border border-slate-200 shadow-2xs">
                ...
                <span className="font-semibold text-slate-900">{d.new_status}</span>
              </div>
            )}
          </div>
          ...
          {d.notes && (
            <div className="bg-white p-3 rounded-xl border border-slate-200/80 shadow-2xs flex items-start space-x-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-800 font-medium">{d.notes}</p>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            ...
            {d.ai_recommendation && (
              <div className="bg-white px-3 py-1.5 rounded-xl border border-slate-200 shadow-2xs">
                ...
                <span className="font-semibold text-slate-900">{d.ai_recommendation}</span>
              </div>
            )}
          </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
              ...
              <span className="text-base font-bold text-slate-900">{d.payments_count ?? '—'}</span>
            </div>
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
              <span className="text-slate-400 text-[10px] uppercase font-bold block">Settlements Ingested</span>
              <span className="text-base font-bold text-slate-900">{d.settlements_count ?? '—'}</span>
            </div>
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
              <span className="text-slate-400 text-[10px] uppercase font-bold block">Fee Records</span>
              <span className="text-base font-bold text-slate-900">{d.fees_count ?? '—'}</span>
            </div>
          </div>

          {Array.isArray(d.files) && d.files.length > 0 && (
            <div className="bg-white p-3 rounded-xl border border-slate-200/80 shadow-2xs flex flex-wrap items-center gap-2 text-xs">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
              ...
              <span className="text-base font-bold text-blue-600">{d.match_rate ?? '—'}</span>
            </div>
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
              <span className="text-slate-400 text-[10px] uppercase font-bold block">Total Records</span>
              <span className="text-base font-bold text-slate-900">{d.total_records ?? '—'}</span>
            </div>
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
              <span className="text-slate-400 text-[10px] uppercase font-bold block">Matched</span>
              <span className="text-base font-bold text-emerald-600">{d.matched_records ?? '—'}</span>
            </div>
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
              <span className="text-slate-400 text-[10px] uppercase font-bold block">Exceptions Flagged</span>
              <span className="text-base font-bold text-amber-600">{d.exceptions_count ?? '—'}</span>
            </div>
          </div>
            ...
            <div className="bg-white p-3 rounded-xl border border-slate-200/80 shadow-2xs space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(breakdown).map(([type, count]) => (
                  <span
                    key={type}
                    className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-50 text-slate-700 border border-slate-200"
                  >
                    <span>{type}:</span>
                    <strong className="text-slate-900">{String(count)}</strong>
                  </span>
                ))}
              </div>
            </div>
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
        {Object.entries(d).map(([key, value]) => {
          ...
          return (
            <div key={key} className="bg-white p-2.5 rounded-xl border border-slate-200 shadow-2xs text-xs">
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-200">
      {/* Filters Bar - Compact */}
      <div className="bg-white border border-slate-200/80 rounded-xl px-3 py-1.5 shadow-2xs">
        <div className="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-2.5">
          <div className="relative w-full xl:w-72 shrink-0">
            ...
            <input
              ...
              className="w-full bg-slate-50/70 border border-slate-200 rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-1 focus:ring-blue-500 transition"
            />
          </div>
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-semibold whitespace-nowrap transition cursor-pointer ${
                  selectedAction === a.id
                    ...
                    : 'bg-slate-50 hover:bg-slate-100/80 text-slate-600 border border-slate-200/80'
                }`}
              >
                {a.label}
              </button>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-230px)] min-h-[300px]">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50/95 sticky top-0 z-10 text-slate-500 font-bold border-b border-slate-200/80 uppercase tracking-wider text-[11px] backdrop-blur-xs">
                <tr>
                ...
                {filteredLogs.map((log) => {
                  ...
                  return (
                    <React.Fragment key={log.id}>
                      <tr className={`hover:bg-slate-50/70 transition ${isExpanded ? 'bg-slate-50/50' : ''}`}>
                        <td className="py-3.5 px-5 font-mono text-slate-500 whitespace-nowrap text-xs">
                          {new Date(log.created_at).toLocaleString()}
                        </td>
                        <td className="py-3.5 px-5 whitespace-nowrap">
                          <ActionBadge action={log.action_type} />
                        </td>
                        <td className="py-3.5 px-5 font-mono text-slate-900 whitespace-nowrap text-xs">
                          <span className="text-slate-400 font-sans text-[10px] mr-1.5 uppercase font-bold tracking-wider">
                            {log.entity_type}
                          </span>
                          {log.entity_id}
                        </td>
                      ...
                      {isExpanded && (
                        <tr className="bg-slate-50/60 border-t border-slate-100">
                          <td colSpan={5} className="py-4 px-6">
                            <div className="bg-slate-50/80 p-4 rounded-2xl border border-slate-200/90 shadow-2xs space-y-3">
                              ...
                              <div className="pt-2 flex items-center justify-end border-t border-slate-200/60">
                                <button
                                  ...
                                  className="inline-flex items-center space-x-1 text-[11px] font-semibold text-slate-500 hover:text-slate-800 transition cursor-pointer"
                                >
